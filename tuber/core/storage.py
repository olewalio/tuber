...
def recompute_content_latest(conn: sqlite3.Connection, content_id: int | None = None) -> int:
    """Пересчитать «последнее известное» состояние из последнего снапшота."""
    where = "WHERE content_id = ?" if content_id is not None else ""
    params = [content_id] if content_id is not None else []
    cur = conn.execute(
        f"""
        ...
        SELECT m.content_id, m.captured_at, m.views, m.likes, m.comments, m.replies,
