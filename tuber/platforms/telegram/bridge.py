...
def existing_handles(con) -> set[str]:
    """Известные хендлы во ВСЕХ статусах в КАНОНИЧЕСКОЙ форме.
    ...
    Ключ — канонический (см. :mod:`tuber.platforms.telegram.linking`), поэтому
    ``@X``, ``t.me/x`` и ``X`` считаются одной записью (ТЗ-51, D-55).
    """
    return {k for k in (linking.normalize_handle(h) for h in db.known_handles(con)) if k}
...
def import_candidates(con, candidates: dict, *, source: str, dry: bool,
    ...
    for raw_handle, info in ordered:
        h = linking.normalize_handle(raw_handle)
        if not h:
