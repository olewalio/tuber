...
def _safe_urlsplit(url: str):
    """``urlsplit`` без исключений: битый URL → ``None`` (ТЗ №2 ч.2).

    Из текста поста регулярка вытаскивает конструкции вида
    ``https://tokenrouter.com](https://tokenrouter.com`` (markdown-ссылка) или
    ``https://[::1``: несбалансированная квадратная скобка в netloc заставляет
    ``urlsplit`` бросить ``ValueError: Invalid IPv6 URL``. Такая ссылка просто
    отбрасывается, пост сохраняется, наружу ничего не летит.
    """
    try:
        return urlsplit(url)
    except ValueError:
...
def _split_external_links(links):
    """Разделить ссылки: (внешние, число битых). Битая — не ошибка парсинга."""
    external, bad = [], 0
    for u in links:
        parsed = _safe_urlsplit(u)
        if parsed is None:
            bad += 1
            continue
...
def parse_message(msg: Node, handle: str, stats=None):
    ...
    ext_links, bad_urls = _split_external_links(links)
    if bad_urls and stats is not None:
        # Один warning на канал за прогон (не заливаем журнал) — см. collect_channel_web.
        handles = stats.setdefault("bad_link_handles", [])
...
def parse_page(html_text: str, handle: str, stats=None):
    """Вернуть список постов, разобранных со страницы t.me/s.

    Служебные сообщения t.me/s (``Channel created``/``photo updated``,
    ``Live stream started``, ``<X> pinned ...`` и подобные) помечены на
    странице классом ``service_message``; они не являются постами и
    отсекаются. Если передан ``stats``, число отсечённых накапливается в
    ``stats["service_skipped"]`` (ТЗ-10 B4).
    """
    ...
    for n in walk(root):
        ...
        if "tgme_widget_message" not in n.cls:
            continue
        if n.has_class("service_message") or n.has_class("tgme_widget_message_service"):
            service += 1
            continue
        p = parse_message(n, handle, stats=stats)
        if p is not None:
            posts.append(p)
    posts.sort(key=lambda p: p["message_id"])
    if stats is not None:
        stats["service_skipped"] = stats.get("service_skipped", 0) + service
    return posts
...
class Collector:
    ...
    def _store_posts(self, channel_id, posts):
        ...
        for p in posts:
            if not p["text"] and not p["has_media"]:
                continue
            exists = con.execute(
                """SELECT views, forwards, reactions, media_kind, text, text_hash,
                          views_checked_at,
                          (views_checked_at IS NULL
                           OR views_checked_at < datetime('now','-1 day')) AS views_due
            ...
            if exists is None:
                ...
                new += 1
                if not self.dry_run:
                    self._record_edges(channel_id, p)
                if p["views"] is not None:
                    touched.append(p["message_id"])
            else:
                # Change-detection (ТЗ-10 B2): UPDATE делаем и «обновлением»
                # считаем только если реально меняется хотя бы одно поле.
                # Повторный прогон по тем же постам не порождает 2-3 тыс.
                # пустых UPDATE и не гоняет зря триггеры posts_upd.
                views_due = bool(exists["views_due"])
                new_views = exists["views"]
                if views_due and p["views"] is not None:
                    # Просмотры платформы монотонны (ТЗ-53): сетевой ответ меньше
                    # уже сохранённого не понижает ряд. Ту же защиту делает
                    # очередь замеров (`metrics.monotonic_views`), поэтому оба
                    # писателя согласованы. `views_checked_at` по-прежнему
                    # принадлежит ТОЛЬКО сборщику (суточный лимит); очередь его
                    # не трогает, она пишет снимки напрямую в `metric_snapshot`.
                    prev_views = int(exists["views"]) if exists["views"] is not None else 0
                    new_views = max(prev_views, int(p["views"]))
                new_forwards = (p["forwards"] if p["forwards"] is not None
                                else exists["forwards"])
                new_reactions = (p["reactions"] if p["reactions"] is not None
                                 else exists["reactions"])
                new_media_kind = (exists["media_kind"] if exists["media_kind"] is not None
                                  else p["media_kind"])
                new_text = exists["text"]
                new_text_hash = exists["text_hash"]
                if ((exists["text"] is None or exists["text"] == "")
                        and p["text"] not in (None, "")):
                    new_text = p["text"]
                    new_text_hash = p["text_hash"]
                if (new_views == exists["views"]
                        and new_forwards == exists["forwards"]
                        and new_reactions == exists["reactions"]
                        and new_media_kind == exists["media_kind"]
                        and new_text == exists["text"]
                        and new_text_hash == exists["text_hash"]):
                    continue
                ...
                con.execute(
                    ...
                    (
                        new_views, p["forwards"], p["reactions"], p["media_kind"],
                        p["text"], p["text"], p["text"], p["text"], p["text"], p["text_hash"],
                        channel_id, p["message_id"],
                    ),
                )
                upd += 1
                # Просмотры обновились только что — этот пост идёт в расчёт
                # дельт/корзины (ТЗ-43, п. 2).
                if views_due and p["views"] is not None:
                    touched.append(p["message_id"])
        if not self.dry_run:
            con.commit()
            self._fill_metric_deltas(channel_id, touched)
        return new, upd

    def _fill_metric_deltas(self, channel_id, message_ids):
        """Заполнить ``bucket``/``delta_*`` свежих снимков Telegram (ТЗ-43).

        Общий расчёт живёт в :mod:`tuber.core.metrics` (раньше — только в
        YouTube-пути). Сбой расчёта не имеет права ронять сбор.
        """
        if not message_ids:
            return
        try:
            ...
            row = self.con.execute(
                "SELECT handle FROM source WHERE id=?", (channel_id,)).fetchone()
            ...
            for mid in message_ids:
                ...
                cid = self.con.execute(
                    ...
                    (f"{handle}/{mid}",)).fetchone()
                if cid is not None:
                    core_metrics.apply_deltas_latest(self.con, int(cid["id"]))
            self.con.commit()
        except Exception:  # noqa: BLE001 — расчёт дельт не роняет сбор
            pass

    def _record_edges(self, channel_id, p):
        """Рёбра одного Telegram-поста (ТЗ-8 Р1.5). Ошибка не роняет сбор."""
        try:
            row = self.con.execute(
                "SELECT handle FROM source WHERE id=?", (channel_id,)).fetchone()
            handle = row["handle"] if row else None
            ...
            core_graph.record_post_edges(
                self.con, platform="telegram", from_content_id=cid[0],
                from_source_id=channel_id, from_handle=handle,
                links=p.get("links"), mentions=p.get("mentions"), origin="collect")
    ...
    def collect_channel_web(self, ch):
        ...
        while True:
            ...
            if first_html is None:
                first_html = text
            parsed = parse_page(text, handle, stats=self.summary)
            if (handle in self.summary.get("bad_link_handles", ())
                    and handle not in self._url_warned):
                self._url_warned.add(handle)
                self.log("info", handle,
                         "битая ссылка отброшена (Invalid IPv6 URL) — пост сохранён")
            if not parsed:
                break
            posts.extend(parsed)
            pages += 1
        ...
        if not self.dry_run:
            ...
            self.con.commit()
        self.log("info", handle,
                 f"ok pages={pages} new={new} upd={upd} "
                 f"service_skipped={self.summary['service_skipped']}")
    ...
    def _mt_message(self, m):
        ...
        return {
            ...
            "reactions": None,
            "media_kind": media_kind,
            "links": json.dumps(_split_external_links(extract_links(text))[0], ensure_ascii=False),
            "hashtags": json.dumps(extract_hashtags(text), ensure_ascii=False),
            "mentions": json.dumps(extract_mentions(text), ensure_ascii=False),
            "fwd_from": fwd_from,
            "is_forward": is_forward,
...
def main(argv=None):
    ...
    try:
        ...
        result = {
            ...
            "posts_new": 0, "posts_upd": 0, "service_skipped": 0, "errors": 1,
