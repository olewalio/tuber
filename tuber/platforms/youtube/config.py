...
# --- Ротация порций поисковых запросов (ТЗ-11, W3-1/W3-2) -----------------

# Сколько запросов берём за один прогон. W3-1: поиск оставлен только для
# обнаружения НОВЫХ каналов, а не для сбора новых видео (это делает дешёвый
# обход uploads, 1 unit против 100). Прежние 40 запросов давали до 8000 units
# за прогон; 2 запроса — до 400 units.
COLLECT_QUERIES_PER_RUN = 2
# W3-1: сколько подтверждённых ИИ-каналов реестра обходим через uploads за
# один прогон. Обход одного канала стоит ~1 unit (плейлист). Если подтверждённых
# каналов больше, действует ротация по курсору (как у запросов).
#
# ТЗ-12 (волна 3.1): дефолт поднят 60 -> 400. Реестр подтверждённых ИИ-каналов
# в боевой базе — 1 608, поэтому при 60/прогон полный круг занимал ~27 суток, а
# суточный цикл один: основной дешёвый канал сбора не покрывал реестр.
# Бюджет: 400 каналов ≈ 400 units за прогон против рабочего порога проекта
# QUOTA_LIMIT_PER_PROJECT - QUOTA_SAFETY_RESERVE = 8000 units (даже 4 прогона
# в сутки дают 1600 units, то есть 16% лимита). Прежний режим (40 поисков по
# 100 units) давал до 8000 units. При 400/прогон полный круг 1608 каналов
# проходится за 4-5 прогонов.
...
# W3-1: отдельный файл состояния ротации обхода uploads (тот же формат, что и
# у запросов): курсор и служебные счётчики фазы не должны пересекаться.
UPLOADS_ROTATION_FILE = "data/uploads_rotation.json"

# Переменная окружения, перекрывающая число запросов на прогон (W3-1).
COLLECT_QUERIES_PER_RUN_ENV = "COLLECT_QUERIES_PER_RUN"

# ТЗ-12: переменная окружения, перекрывающая число каналов обхода uploads на
# прогон (по образцу COLLECT_QUERIES_PER_RUN_ENV). Читается в момент вызова,
# поэтому работает и после load_env; битое/пустое значение молча пропускается.
COLLECT_UPLOADS_CHANNELS_PER_RUN_ENV = "COLLECT_UPLOADS_CHANNELS_PER_RUN"

# --- W3-2: дедуп поисковых запросов и учёт их отдачи ----------------------

# Незначащие слова, которые не учитываются при сравнении формулировок
# («... AI», «AI ...», «новости ...»). Сами запросы НЕ меняются: стоп-набор
# нужен только чтобы свести близкие формулировки в один класс эквивалентности.
POOL_STOPWORDS: frozenset[str] = frozenset({"ai", "ии", "и", "новости", "news"})
# Сколько прогонов подряд запрос должен дать found=0, чтобы выпасть из ротации.
QUERY_ZERO_YIELD_LIMIT = 3
# На сколько дней запрос выводится из ротации после серии пустых прогонов.
QUERY_QUARANTINE_DAYS = 14

# Регулярка для удаления пунктуации при нормализации запроса.
_QUERY_PUNCT_RE = re.compile(r"[^\w\s]", re.UNICODE)


def normalize_query_key(query: str) -> str:
    """Ключ эквивалентности поискового запроса (W3-2, только для сравнения).

    Приводит регистр к нижнему, схлопывает пробелы, удаляет пунктуацию и
    незначащие слова из :data:`POOL_STOPWORDS`. Формулировки с одинаковым
    ключом считаются дублями; сами запросы не переписываются.
    """
    text = str(query).strip().lower().replace("ё", "е")
    text = _QUERY_PUNCT_RE.sub(" ", text)
    tokens = [tok for tok in text.split() if tok and tok not in POOL_STOPWORDS]
    return " ".join(tokens)


def dedupe_search_queries(queries: list[str]) -> tuple[list[str], list[str]]:
    """Свести пул к одному представителю на класс эквивалентности (W3-2).

    Порядок пула сохраняется: остаётся первая формулировка класса. Возвращает
    ``(kept, dropped)``: очищенный пул и список отсечённых дублей (для лога).
    """
    kept: list[str] = []
    dropped: list[str] = []
    seen: set[str] = set()
...
def collect_queries_per_run(cfg: Any = None) -> int:
    """Действующее число поисковых запросов на прогон (W3-1).

    Приоритет: переменная окружения COLLECT_QUERIES_PER_RUN (читается в момент
    вызова, поэтому работает и после load_env), затем одноимённый атрибут cfg,
    затем константа модуля. Некорректное значение молча пропускается.
    """
    env = _env_positive_int(COLLECT_QUERIES_PER_RUN_ENV)
    if env is not None:
        return env
    if cfg is not None:
        fallback = getattr(cfg, "COLLECT_QUERIES_PER_RUN", None)
        if fallback is not None:
            try:
                ...
                pass
    return int(COLLECT_QUERIES_PER_RUN)


def collect_uploads_channels_per_run(cfg: Any = None) -> int:
    """Действующее число каналов обхода uploads на прогон (ТЗ-12).

    Приоритет: переменная окружения COLLECT_UPLOADS_CHANNELS_PER_RUN (читается
    в момент вызова, поэтому работает и после load_env), затем одноимённый
    атрибут cfg, затем константа модуля. Некорректное/пустое значение env молча
    пропускается — тихий откат на конфиг, прогон не ломается.
    """
    env = _env_positive_int(COLLECT_UPLOADS_CHANNELS_PER_RUN_ENV)
    if env is not None:
        return env
    if cfg is not None:
        fallback = getattr(cfg, "COLLECT_UPLOADS_CHANNELS_PER_RUN", None)
        if fallback is not None:
            try:
                value = int(fallback)
                if value >= 0:
                ...
                pass
    return int(COLLECT_UPLOADS_CHANNELS_PER_RUN)
...
# Сколько видео разбирать за прогон команды comments. Это число ВИДЕО, а не
# обращения к API: на видео теперь может уйти несколько страниц (см.
# COMMENT_PAGES_PER_VIDEO). Суточный прогон берёт топ-50 виральных видео,
# фактические обращения к commentThreads — до COMMENT_MAX_VIDEOS_PER_RUN ×
# COMMENT_PAGES_PER_VIDEO (квота показывается числом units в отчёте, ТЗ-49).
COMMENT_MAX_VIDEOS_PER_RUN = 50
# Сколько верхних комментариев брать с одного видео (отбор по лайкам).
# Поднято с 20 до 500 (ТЗ-49): приёмка требует ≥ 20 000 комментариев, для чего
# одного «верхнего двадцатка» недостаточно. Хранится не больше, чем пришло.
...
# Сколько страниц commentThreads тянуть с одного видео за прогон (ТЗ-49).
# 1 unit за страницу до 100 комментариев; пагинация через nextPageToken. Обход
# останавливается раньше, если API не отдал следующую страницу.
COMMENT_PAGES_PER_VIDEO = 5
# Сколько виральных видео суточного топа разбирать (ТЗ-49): топ-50.
COMMENT_VIRAL_TOP_N = 50
# Окно, за которое берётся виральный топ для отбора видео под комментарии.
COMMENT_VIRAL_WINDOW_DAYS = 14
# Сколько каналов из топа разбирать по их свежим видео (ТЗ-49): топ-3.
COMMENT_CHANNEL_TOP_N = 3
# Сколько последних видео топ-канала просматривать на комментарии.
COMMENT_CHANNEL_VIDEOS = 5
# Порог лайков, с которого автор комментария попадает в очередь кандидатов
# (ТЗ-49): пишущие по делу, а не весь шум.
COMMENT_HARVEST_MIN_LIKES = 50
# Окно свежести для вирального суточного обхода (часы). Отдельно от
# COMMENT_REFRESH_DAYS, чтобы вернуть суточный ритм из плана: один и тот же
# топ пересматривается раз в сутки, но повторные комментарии не дублируются
# (INSERT OR IGNORE).
...
def load_key_projects(
    ...
    * с привязкой: ``{"keys": [{"key": "AIza...", "project": "123456789012"},
