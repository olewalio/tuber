...
class YouTubeClient:
    ...
    def comment_threads(self, video_id: str, max_results: int = 20) -> list[dict]:
        ...
        Совместимая обёртка над :meth:`comment_threads_page`: возвращает ровно
        первый страничный набор полей (без ``author_channel_id``), как раньше.
        Для расширенного сбора (пагинация + идентификатор канала автора)
        используйте :meth:`comment_threads_page`.
        """
        items, _next = self.comment_threads_page(
            video_id, max_results=max_results, page_token=None,
        )
        out = []
        for it in items:
            it = dict(it)
            it.pop("author_channel_id", None)
            out.append(it)
        return out
    ...
    def comment_threads_page(
        ...
        """Одна страница верхних комментариев + токен следующей.

        Возвращает ``(items, next_page_token)``. Поля каждого комментария:
        comment_id, author, author_channel_id (UC-идентификатор канала автора,
        может быть None у удалённых/скрытых), text, likes, published_at.

        Стоимость вызова — 1 unit независимо от ``maxResults`` (до 100). 403
        уровня видео поднимается как :class:`YouTubeError` с кодом 403, ключ
        остаётся рабочим (см. :meth:`comment_threads`).
        """
        params: dict[str, Any] = {
            "part": "snippet",
            "videoId": video_id,
