...
def open_readonly(db_path: str | Path | None = None) -> sqlite3.Connection:
    """Открыть единую базу ядра ТОЛЬКО для чтения и поставить слой совместимости.

    Публичная точка входа для инструментов, читающих боевую базу без права
    записи (например ``tuber yt candidates-export``). Раньше такие места
    открывали файл «сырым» ``sqlite3.connect("file:...?mode=ro")`` и не видели
    legacy-имён (``videos``, ``channels``, …): в схеме ядра v5 это TEMP-
    представления, которые создаёт единственный код — :func:`install_compat`.
    Отсюда ``no such table: videos`` на боевой базе.

    Гарантии:

    * файл открывается по URI ``file:<path>?mode=ro`` (см.
      :func:`tuber.core.db.connect`): запись в main-базу физически невозможна —
      «attempt to write a readonly database»;
    * TEMP-представления/триггеры совместимости создаются тем же
      :func:`install_compat`, что и в :func:`connect` (одна точка правды, без
      копий DDL);
    * ``migrate_schema`` не вызывается вовсе: он пишет в main, а база read-only.

    Почему снимается ``PRAGMA query_only`` на время установки слоя. Ядровой
    :func:`tuber.core.db.connect(readonly=True)` выставляет ``query_only=ON``.
    SQLite 3.45 при этом запрещает и создание TEMP-объектов (проверено:
    ``CREATE TEMP VIEW`` → «attempt to write a readonly database»), а слой
    совместимости — это именно TEMP-представления. Безопасность от этого не
    страдает: соединение открыто ``mode=ro``, и запись в main всё равно
    невозможна (это подтверждается тестом). После установки слоя ``query_only``
    возвращается — двойной предохранитель остаётся на месте.
    """
    path = Path(db_path) if db_path is not None else config.DB_PATH
    core_sqlcompat.ensure_supported_sqlite()
    conn = core_db.connect(str(path), readonly=True, factory=_CompatConnection)
    try:
