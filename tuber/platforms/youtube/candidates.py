...
def open_readonly(path: str | Path) -> sqlite3.Connection:
    """Открыть БД только для чтения и поставить слой совместимости.

    Делегирует в :func:`tuber.platforms.youtube.store.open_readonly`: раньше
    здесь был голый ``sqlite3.connect("file:...?mode=ro")`` без TEMP-
    представлений, и ``export_external`` падал на боевой базе с
    ``no such table: videos``. Единственная точка правды о слое совместимости —
    :func:`store.install_compat`; копий DDL здесь нет.
    """
