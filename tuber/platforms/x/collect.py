...
#: Признак «у поста есть метрики сессионного транспорта» (ТЗ-43A/43B).
#: У постов Nitter RSS этих ключей нет — значит снапшоты они не создают.
SESSION_METRIC_KEYS = ("views", "likes", "reposts", "quotes")


def new_metrics_state():
    """Общая для одного прогона «касса» метрик сессии X (ТЗ-43B).

    ``seen`` — tweet_id, уже принятые в этом прогоне (дедупликация);
    ``written`` — сколько записей принято (для потолка);
    ``capped`` — писали ли уже строку про потолок в run_log.
    """
    return {"seen": set(), "written": 0, "capped": False}


def _has_session_metrics(post) -> bool:
    return any(post.get(k) is not None for k in SESSION_METRIC_KEYS)


#: Сколько tweet_id брать в один запрос гидратации (как это делает сам X).
SESSION_METRICS_BATCH = 100


def enrich_session_metrics(posts):
    """ТЗ-43C: дотянуть метрики постов через сессию X (точечно, батчами).

    Ленты через сессию по умолчанию ВЫКЛЮЧЕНЫ (``X_SESSION_FEDS_ENABLED=0``),
    поэтому метрики постов берём точечной операцией ``TweetResultsByRestIds``
    на пачку id. Посты, у которых метрики уже есть (лента шла через сессию),
    не трогаем. Любой отказ (нет сессии, cooldown, бюджет, сеть) — молча выходим:
    метрики не имеют права ронять сбор. Бюджет считает сам транспорт по журналу.
    """
    if not posts or not getattr(config, "X_SESSION_ENABLED", False):
        return posts
    need = [p for p in posts
            if p.get("tweet_id") and not _has_session_metrics(p)]
    if not need:
        return posts
    try:
        from .session import get_session_transport
        sess = get_session_transport()
    except Exception:  # noqa: BLE001 — нет сессии = нет метрик, не отказ сбора
        return posts
    if sess is None:
        return posts
    found = {}
    for i in range(0, len(need), SESSION_METRICS_BATCH):
        batch = [str(p["tweet_id"]) for p in need[i:i + SESSION_METRICS_BATCH]]
        try:
            infos = sess.tweets_by_ids(batch)
        except Exception:  # noqa: BLE001 — бюджет/cooldown/сеть: без метрик
            break
        for info in infos or []:
            if info.get("id"):
                found[str(info["id"])] = info
    for p in need:
        info = found.get(str(p["tweet_id"]))
        if not info:
            continue
        p["views"] = info.get("views")
        p["likes"] = info.get("likes")
        p["replies"] = info.get("replies")
        p["reposts"] = info.get("retweets")
        p["quotes"] = info.get("quotes")
    return posts


def _accept_session_metric(con, tid, state, cap, run_id):
    """ТЗ-43B: решить, писать ли метрики поста в этом прогоне.

    Не больше одной записи на ``tweet_id`` за прогон (дедупликация) и не
    больше ``cap`` записей всего. При упоре в потолок — одна понятная строка
    по-русски в ``run_log``; дальше метрики прогона не принимаются.
    Возвращает ``True``, если пост принят.
    """
    ...
    if state["written"] >= cap:
        if not state["capped"]:
            ...
            try:
                db.log_run(
                    con, "WARN",
                    f"ТЗ-43B: потолок метрик сессии X за прогон достигнут"
                    f" ({cap} записей) — остальные метрики прогона не пишутся",
                    run_id=run_id)
            except Exception:  # noqa: BLE001 — лог не имеет права ронять сбор
                pass
        return False
    state["seen"].add(tid)
    state["written"] += 1
...
def _record_post_edges(con, tid, account_id, handle, post):
    """Записать рёбра одного X-поста (ТЗ-8 Р1.5). Ошибка не должна ронять сбор."""
    try:
        row = con.execute(
        ...
        if row is None:
            return
        core_graph.record_post_edges(
            con, platform="x", from_content_id=row["id"], from_source_id=account_id,
            from_handle=post.get("owner_handle") or handle,
            links=post.get("links"), mentions=post.get("mentions"),
            orig_handle=post.get("orig_handle"),
            is_quote=bool(post.get("is_quote")), is_repost=bool(post.get("is_retweet")),
            origin="collect")
...
def store_posts(con, account_id, posts, *, dry_run=False, text_src="nitter",
                handle=None, run_id=None, metrics_state=None):
    ...
    ТЗ-8 Р1.5: на каждый пост пишутся рёбра графа (`edge`) — ссылки, упоминания,
    цитата/репост (автор оригинала известен ТОЛЬКО здесь, из парсера). Ноль
    сетевых запросов.

    ТЗ-43B: метрики сессионного транспорта пишутся на КАЖДЫЙ пост прогона, у
    которого они есть (и `new`, и `upd`) — иначе ряд значений по времени у
    существующего поста не наполняется. `metrics_state` (см.
    :func:`new_metrics_state`) даёт дедупликацию по `tweet_id` и потолок
    `config.X_SESSION_METRICS_MAX_PER_RUN` на весь прогон: один и тот же пост не
    пишется дважды, при упоре в потолок в `run_log` идёт строка по-русски. У
    постов Nitter RSS метрик-ключей нет — они снапшотов не создают.
    """
    new = upd = skipped = 0
    session_posts = []
    metrics_cap = int(getattr(config, "X_SESSION_METRICS_MAX_PER_RUN", 3000))
    if metrics_state is None:
        metrics_state = new_metrics_state()
    if handle is None and account_id is not None:
        row = con.execute("SELECT handle FROM source WHERE id=?", (account_id,)).fetchone()
    ...
    for p in posts:
        ...
        if exists:
            ...
            if db.changes_since(con, before) > 0:
                ...
                upd += 1
        # ТЗ-8 Р1.5: рёбра поста — после того, как он получил content.id.
        if not dry_run:
            _record_post_edges(con, tid, account_id, handle, p)
        # ТЗ-43B: метрики сессионного транспорта — и для новых, и для уже
        # известных постов (диагноз ТЗ-43B: upd-ветка их не собирала, поэтому
        # ряд просмотров у существующего поста не наполнялся). Дедупликация по
        # tweet_id и общий потолок на прогон — в `metrics_state`.
        if (not dry_run) and _has_session_metrics(p):
            if _accept_session_metric(con, tid, metrics_state, metrics_cap,
                                      run_id):
                session_posts.append(p)
    if not dry_run:
        # не держим открытую транзакцию: брокеру тоже надо писать в БД
        con.commit()
        # ТЗ-43A/43B: метрики постов сессионного транспорта — в metric_snapshot.
        if session_posts:
            try:
                db.record_session_metrics(con, session_posts)
            except Exception:  # noqa: BLE001 — метрики не роняют сбор
                pass
...
def collect_tier(con, broker, tier, *, max_accounts=None, dry_run=False,
    ...
    max_reserve = int(getattr(config, "XSSR_MAX_PER_RUN", config.XSSR_MAX_PER_RUN))
    ...
    reserve_used = 0
    # ТЗ-43B: общая на прогон «касса» метрик сессии X — дедупликация и потолок
    # X_SESSION_METRICS_MAX_PER_RUN на все аккаунты прогона.
    metrics_state = new_metrics_state()
    ...
    for account in accounts:
        ...
        try:
            ...
            if posts:
                posts = enrich_session_metrics(posts)
                res = store_posts(con, account["id"], posts, text_src=None,
                                  run_id=run_id, metrics_state=metrics_state)
    ...
    parts = []
    if reserve_used:
        thr = int(getattr(config, "XSSR_DEGRADED_STREAK", config.XSSR_DEGRADED_STREAK))
...
def backfill(con, broker, handle, pages=3, *, run_id=None):
    ...
    cursor = row["cursor"]
    seen = set()
    # ТЗ-43B: метрики и здесь под общим потолком прогона.
    metrics_state = new_metrics_state()
    ...
    for i in range(int(pages)):
        ...
        res = store_posts(con, row["id"], fresh, run_id=run_id,
...
def refetch_fulltext(con, broker, *, router=None, limit=None, run_id=None,
    ...
    summary = {"before": before, "targets": len(targets), "accounts": 0,
               ...
               "new": 0, "upd": 0, "dry_run": dry_run, "details": []}
    ...
    # ТЗ-43B: метрики и в доборе текста под общим потолком прогона.
    metrics_state = new_metrics_state()
    for aid in acc_ids:
        ...
        if ssr:
            res = store_posts(con, aid, ssr, text_src=None, run_id=run_id,
                              metrics_state=metrics_state)
            summary["ssr_used"] += 1
            summary["new"] += res["new"]
            summary["upd"] += res["upd"]
