...
REPORT_JOB_SCRIPT = "tuber_x_report.sh"  # имя скрипта задания отчёта (Р1.1)
...
def _report_job_ids(config_path, script):
    ...
    например `tuber_x_report.sh`. Возвращает список id (обычно один).
    """
    ...
    for job in jobs or []:
        ...
        if isinstance(field, str) and _os.path.basename(field) == script:
            jid = job.get("id")
            if jid is not None:
                ids.append(str(jid))
    return ids
...
def check_report_delivery(con=None, hours=None, *, log_dir=None, config_path=None,
    ...
    if not job_ids:
        return _res("report_delivery", True,
                    ...
                    f" планировщика нет задания со скриптом {script}",
