...
def _safe_urlparse(value):
    """``urlparse`` без исключений: битый netloc (Invalid IPv6 URL) → None."""
    try:
...
def _extract_links(html_fragment, host=None):
    ...
    host_key = None
    if host:
        _hp = _safe_urlparse(host)
        host_key = _hp.netloc.lower() if _hp is not None else None
...
def _extract_mentions(html_fragment, text, host=None):
    ...
    for href in re.findall(r"""href=["']([^"']+)["']""", html_fragment or ""):
        p = _safe_urlparse(html.unescape(href))
        if p is None:
            continue
        if host_key and p.netloc.lower() != host_key:
            continue
...
class NitterBroker:
    ...
    def __init__(self, instances=None, *, db_path=None, transport=None,
                 ...
                 run_id=None, cache_ttl=None, daily_cap=None, session=None):
        ...
        self.run_id = run_id
        # ТЗ-43A: свой сессионный транспорт X — первый канал для `feed`.
        # `None` = попытаться создать лениво; `False` = не использовать вовсе.
    ...
    def _fetch(self, kind, key, cursor, priority, force):
        ...
        if cached is not None:
            return cached
        # ТЗ-43C: сессия обслуживает ТОЧЕЧНЫЕ задачи (followers, метрики постов),
        # а ленты через неё по умолчанию ВЫКЛЮЧЕНЫ (X_SESSION_FEDS_ENABLED=0):
        # именно сплошной поток лент 20 зап/мин родил 429. Любой отказ
        # (нет сессии / лимит / бюджет / ошибка) — молчаливый откат на Nitter,
        # при этом счётчики отказов Nitter-инстансов НЕ трогаются.
        if kind == "feed" and config.X_SESSION_FEDS_ENABLED:
            posts = self._try_session_feed(key)
            if posts is not None:
                self._cache_put(ckey, posts)
                return posts
    ...
    # ----------------------------------------------- свой транспорт X (ТЗ-43A)
    def _session_transport(self):
        """Лениво создать сессионный транспорт. ``None`` — сессия недоступна.

        Отсутствие сессии кешируется на жизнь процесса: это не отказ Nitter и
        не должно повторяться на каждом запросе.
        """
        if self._session is False:
            return None
        if self._session is None:
            from .session import get_session_transport, session_last_error
            self._session = get_session_transport(db_path=self._db_path,
                                                  run_id=self.run_id)
            if self._session is None:
                self._session = False
                self._warn(None, "сессия X недоступна, работаем на Nitter: "
                                 f"{session_last_error()}")
        return self._session

    def _try_session_feed(self, handle):
        """Лента через сессию X или ``None`` (тогда путь Nitter не меняется)."""
        sess = self._session_transport()
        if sess is None:
            return None
        try:
            posts = sess.fetch_feed(handle)
        except Exception as e:  # noqa: BLE001 — любой отказ = откат на Nitter
            self._warn(handle, f"сессионный транспорт X отказал, фолбэк на Nitter: "
                               f"{type(e).__name__}: {e}")
            return None
        if not posts:
            return None
        # ТЗ-43B: метрики сессии пишет ТОЛЬКО collect.store_posts (и для новых, и
        # для уже известных постов) — там на весь прогон работает дедупликация по
        # tweet_id и потолок X_SESSION_METRICS_MAX_PER_RUN. Раньше запись была и
        # здесь: вне collect она обходила потолок и не дедуплицировалась. Брокер
        # остаётся слоем транспорта и БД метрик не трогает.
        # Успех сессии — признак здоровья транспорта x.com в transport_instance.
        try:
            db.update_transport_instance(self._con, config.X_SESSION_HOST,
                                         healthy=1, rss_ok=1, blocked=0,
                                         last_check_at=db.utcnow_iso(),
                                         last_error=None)
        except Exception:
            pass
