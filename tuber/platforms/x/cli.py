...
def cmd_registry_verify(args):
    ...
    if handle:
        ...
        res = registry.verify_account(con, broker, handle, run_id=run_id, router=router)
    ...
    print(f"проверено={res['checked']} active={res['active']} "
          ...
          f"retry={res.get('retry', 0)} в_стоп-лист={res['blocked']}")
    if res["reasons"]:
...
def cmd_discover(args):
    ...
    if args.report:
        discover.report(con)
        # ТЗ-8 Р5.1: `x discover --report` печатает и граф источников.
        from tuber.core import graph as _graph
        print()
        print(_graph.format_report(con))
        con.close()
        return 0
    router = _router(args)
    broker = router.nitter
    ...
    if not summary["skipped"] and not args.no_verify:
        ...
        print(f"верификация: проверено={verify_res['checked']} "
              ...
              f"rejected={verify_res['rejected']} retry={verify_res.get('retry', 0)} "
              f"в_стоп-лист={verify_res['blocked']}")
...
def cmd_fulltext(args):
    ...
    if not summary["dry_run"]:
        db.finish_run(con, run_id, accounts_ok=summary["nitter_ok"] + summary["ssr_used"],
                      ...
                      posts_new=summary["new"], posts_upd=summary["upd"],
...
# ------------------------------------------------------------------- budget
def _secs_left(iso_value, now):
    """Сколько секунд до момента ``iso_value`` (None → None)."""
    dt = db.parse_iso(iso_value)
    if dt is None:
        return None
    return max(0, int((dt - now).total_seconds()))


def _print_session_budget(con):
    """ТЗ-43C: сводка бюджета сессии X — окно, сутки, cooldown (по БД)."""
    b = db.session_budget(con)
    st = db.read_session_state(con)
    now = datetime.now(timezone.utc)
    print(f"=== бюджет сессии X (host={b['host']}) на {b['now']} UTC ===")
    print(f"  за сутки (24 ч): {b['daily_count']} из {b['daily_cap']}"
          + ("  [ИСЧЕРПАН]" if b["daily_exhausted"] else ""))
    print(f"  за окно ({b['window_sec']} с): {b['window_count']} из {b['window_cap']}"
          + ("  [ИСЧЕРПАНО]" if b["window_exhausted"] else ""))
    if b["window_free_at"]:
        left = _secs_left(b["window_free_at"], now)
        print(f"  окно освободится: {b['window_free_at']} UTC (через {left} с)")
    else:
        print("  окно освободится: не ограничено (в окне нет записей)")
    if b["daily_free_at"]:
        left = _secs_left(b["daily_free_at"], now)
        print(f"  суточный лимит откроется: {b['daily_free_at']} UTC (через {left} с)")
    cd = st.get("cooldown_until")
    if cd is not None and cd > now:
        left = int((cd - now).total_seconds())
        print(f"  cooldown: до {db.iso(cd)} UTC (ещё {left} с)"
              + (", blocked" if st.get("blocked") else ""))
    else:
        print("  cooldown: нет")
    print(f"  подряд 429: {st.get('consecutive_429', 0)}"
          f" | blocked: {'да' if st.get('blocked') else 'нет'}")
    return 0
...
def cmd_budget(args):
    ...
    if getattr(args, "session", False):
        try:
            return _print_session_budget(con)
        finally:
            con.close()
...
# ----------------------------------------------------- подписчики X (ТЗ-44)
def _avg_views(con, source_id, limit=20):
    """Средние просмотры последних снапшотов аккаунта (None, если данных нет)."""
    row = con.execute(
        "SELECT AVG(v) AS a FROM ("
        "  SELECT m.views AS v FROM metric_snapshot m"
        "  JOIN content c ON c.id = m.content_id"
        "  WHERE c.platform='x' AND c.source_id=? AND m.views IS NOT NULL"
        "  ORDER BY m.captured_at DESC LIMIT ?)",
        (source_id, limit)).fetchone()
    if row is None or row["a"] is None:
        return None
    return int(round(row["a"]))


def _hhmmss(iso_ts):
    """``2026-09-21T12:32:10`` → ``12:32:10`` (для строки отложенных)."""
    if not iso_ts:
        return None
    s = str(iso_ts).replace(" ", "T")
    return s[11:19] if "T" in s else s


def cmd_followers(args):
    """Снимки подписчиков X на сессии (ТЗ-44): subs/subs_at + ряд в meta_json.

    ТЗ-44C: исчерпание бюджета окна/суток — это НЕ отказ аккаунта. Такие
    аккаунты «отложены» до освобождения окна, а не записаны в отказы, и в Nitter
    за ними не ходим (подписчиков там нет). Код возврата: 0 — боевых отказов
    нет (даже если часть отложена), 1 — настоящие отказы, 2 — сессия недоступна.
    """
    from .session import (SessionBlocked, SessionUnavailable, XSessionDailyCap,
                          XSessionError, XSessionRateLimited, XSessionWindowCap,
                          get_session_transport)
    con = db.connect(config.DB_PATH)
    try:
        if args.handle:
            rows = list(con.execute(
                "SELECT id, handle FROM source WHERE platform='x' AND handle=?",
                (args.handle.lstrip("@"),)))
        else:
            rows = list(con.execute(
                "SELECT id, handle FROM source WHERE platform='x'"
                " AND status IN ('active','provisional','candidate')"
                " ORDER BY (subs IS NULL) DESC, subs_at ASC, handle ASC LIMIT ?",
                (int(args.limit),)))
        if not rows:
            print("подписчики X: в реестре нет подходящих аккаунтов")
            return 0
        session = get_session_transport(db_path=config.DB_PATH, run_id=None)
        if session is None:
            print("подписчики X: сессия недоступна, обновление невозможно")
            return 2
        ok = fail = 0
        deferred = 0
        defer_kind = None
        ...
        for row in rows:
            handle = row["handle"]
            try:
                prof = session.followers(handle)
            except (XSessionDailyCap, XSessionWindowCap) as e:
                # Бюджет исчерпан штатно: НЕ отказ, в Nitter не идём (ТЗ-44C).
                defer_kind = ("бюджет суток" if isinstance(e, XSessionDailyCap)
                              else "бюджет окна")
                b = db.session_budget(con)
                free = (b["daily_free_at"] if isinstance(e, XSessionDailyCap)
                        else b["window_free_at"])
                defer_until = _hhmmss(free)
                deferred = len(rows) - ok - fail
                break
            except (SessionUnavailable, SessionBlocked, XSessionRateLimited,
                    XSessionError) as e:
                print(f"  @{handle}: отказ транспорта ({type(e).__name__})")
                fail += 1
                break
            if not prof or prof.get("followers") is None:
                print(f"  @{handle}: профиль не найден")
                fail += 1
                continue
            avg = _avg_views(con, row["id"])
            db.set_follower_snapshot(con, row["id"], prof["followers"], avg_views=avg)
            if prof.get("id"):
                con.execute(
                    "UPDATE source SET external_id=?"
                    " WHERE id=? AND (external_id IS NULL OR external_id='')",
                    (str(prof["id"]), row["id"]))
                con.commit()
            ok += 1
            print(f"  @{handle}: подписчиков {prof['followers']}"
                  + (f", avg_views {avg}" if avg is not None else ""))
        tail = ""
        if deferred > 0:
            when = f", продолжение после {defer_until} UTC" if defer_until else ""
            tail = f" ({defer_kind}{when})"
        print(f"подписчики X: обновлено {ok}, отложено {deferred}{tail},"
              f" отказов {fail}")
...
def build_parser():
    ...
    sp = sub.add_parser("followers",
                        help="снимки подписчиков X на сессии (ТЗ-44)")
    sp.add_argument("--limit", type=int, default=100,
                    help="сколько аккаунтов обновить за прогон")
    sp.add_argument("--handle", default=None,
                    help="обновить только один аккаунт")
    sp.set_defaults(func=cmd_followers)
    ...
    sp.add_argument("--session", action="store_true",
                    help="бюджет сессии X (окно/сутки/cooldown, ТЗ-43C)")
