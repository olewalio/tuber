...
def set_reserve_since(con, when=None, host=None):
    """Установить/снять метку ``reserve_since`` (``None`` — снять флаг).

    ТЗ-Tuber ч.3: метка снимается не только «первым успешным фидом» в том же
    процессе (``clear_reserve_active``), но и сторожем по факту: если за окно
    есть успешные фиды Nitter, флаг, залипший из-за смены процесса, сбрасывается
    именно здесь. Запись — безусловная (в отличие от ``mark_reserve_active``,
    который ставит метку только если её ещё нет).
    """
    host = host or config.XSSR_HOST
    with write_tx(con):
        con.execute(
        ...
        con.execute(
            "UPDATE main.transport_instance"
            " SET meta_json=json_set(COALESCE(meta_json,'{}'), '$.reserve_since', ?)"
            " WHERE platform='x' AND host=?", (when, host))
    return when
...
def clear_reserve_active(con, host=None):
    ...
    host = host or config.XSSR_HOST
    return set_reserve_since(con, None, host)
...
# ===================================================== свой транспорт X (ТЗ-43A/44)
# Состояние и журнал запросов идут через ТЕ ЖЕ представления, что и у Nitter
# (`instances`/`requests` → `transport_instance`/`transport_request`, host='x.com').
# Новых таблиц нет.

def update_transport_instance(con, host, *, platform="x", **fields):
    """Обновить состояние транспорта (host='x.com' для сессии X).

    Пишет через TEMP-представление ``instances`` — тот же путь, что у Nitter.
    Строка создаётся при отсутствии (INSERT OR IGNORE).
    """
    con.execute("INSERT OR IGNORE INTO instances (host, healthy) VALUES (?, NULL)",
                (host,))
    if fields:
        cols = ", ".join(f"{k}=?" for k in fields)
        con.execute(f"UPDATE instances SET {cols} WHERE host=?",
                    (*fields.values(), host))
    con.commit()


def record_transport_request(con, host, kind, url, status, items=0, latency_ms=0,
                             run_id=None):
    """Записать запрос транспорта (``requests`` → ``transport_request``)."""
    con.execute(
        "INSERT INTO requests (host, ts, kind, url, status, items, latency_ms, run_id)"
        " VALUES (?,?,?,?,?,?,?,?)",
        (host, now_iso(), kind, url, status, items, latency_ms, run_id))
    con.commit()


# ------------------------------------------------------- состояние сессии X (ТЗ-43C)
# Зачем поле, а не новая таблица: ``transport_instance`` уже несёт колонки
# ``cooldown_until``/``rate_limited_429``/``blocked`` и уже отдаётся через
# представление ``instances`` с host='x.com'. Новых сущностей в ядре не плодим
# (ТЗ-43C разрешает «поле в существующей transport_instance»). Дополнительно
# метка времени последнего 429 живёт в ``meta_json.last_429_at`` — она нужна,
# чтобы счётчик «подряд» не переживал сутки.
_UNSET = object()
...
def read_session_state(con, host=None):
    """Состояние предохранителей сессии X из БД (ТЗ-43C).

    Читается КАЖДЫЙ раз перед запросом, поэтому значение — общее для всех
    процессов, а не «своё» у каждого конструктора.
    """
    host = host or config.X_SESSION_HOST
    row = con.execute(
        "SELECT cooldown_until, rate_limited_429, blocked, meta_json"
        " FROM main.transport_instance WHERE platform='x' AND host=?",
        (host,)).fetchone()
    if row is None:
        return {"cooldown_until": None, "consecutive_429": 0, "blocked": False,
                "last_429_at": None}
    meta = {}
    try:
    ...
    return {
        ...
        "consecutive_429": int(row["rate_limited_429"] or 0),
        "blocked": bool(row["blocked"]),
        "last_429_at": parse_iso(meta.get("last_429_at")),
    }


def write_session_state(con, host=None, *, cooldown_until=_UNSET,
                        consecutive_429=_UNSET, blocked=_UNSET,
                        last_429_at=_UNSET, last_error=_UNSET):
    """Записать состояние предохранителей сессии X (ТЗ-43C).

    Незаданные поля не трогаются. ``cooldown_until`` — абсолютное время UTC
    (datetime или ISO-строка), пишется в ``transport_instance.cooldown_until``.
    """
    host = host or config.X_SESSION_HOST
    fields = {}
    if cooldown_until is not _UNSET:
        fields["cooldown_until"] = _iso_or_none(cooldown_until)
    if consecutive_429 is not _UNSET:
        fields["rate_limited_429"] = int(consecutive_429)
    if blocked is not _UNSET:
        fields["blocked"] = 1 if blocked else 0
    if last_error is not _UNSET:
        fields["last_error"] = last_error
    if fields:
        update_transport_instance(con, host, **fields)
    if last_429_at is not _UNSET:
        con.execute(
            "UPDATE main.transport_instance"
            " SET meta_json=json_set(COALESCE(meta_json,'{}'), '$.last_429_at', ?)"
            " WHERE platform='x' AND host=?",
            (_iso_or_none(last_429_at), host))
        con.commit()


def session_budget(con, *, now=None, host=None):
    """Бюджет запросов сессии X по ЖУРНАЛУ ``transport_request`` (ТЗ-43C).

    Считает только ``kind='x_session'``: журнал ведут ВСЕ процессы, поэтому
    бюджет общий по построению. Возвращает числа за окно и за сутки плюс
    моменты, когда лимит снова откроется.
    """
    host = host or config.X_SESSION_HOST
    now_iso_s = now_iso() if now is None else iso(now)
    now_dt = parse_iso(now_iso_s) or utcnow()
    window_start = iso(now_dt - timedelta(seconds=config.X_SESSION_WINDOW_SEC))
    day_start = iso(now_dt - timedelta(hours=24))

    def _window(since):
        row = con.execute(
            "SELECT COUNT(*) AS n, MIN(ts) AS oldest FROM main.transport_request"
            " WHERE platform='x' AND host=? AND kind='x_session' AND ts>=?",
            (host, since.replace("T", " "))).fetchone()
        return int(row["n"] or 0), row["oldest"]

    window_n, window_oldest = _window(window_start)
    day_n, day_oldest = _window(day_start)
    window_free = None
    if window_oldest:
        window_free = iso(parse_iso(window_oldest)
                          + timedelta(seconds=config.X_SESSION_WINDOW_SEC))
    day_free = None
    if day_oldest:
        day_free = iso(parse_iso(day_oldest) + timedelta(hours=24))
    return {
        "host": host,
        "now": now_iso_s,
        "window_count": window_n,
        "window_cap": int(config.X_SESSION_WINDOW_CAP),
        "window_sec": int(config.X_SESSION_WINDOW_SEC),
        "window_free_at": window_free,
        "daily_count": day_n,
        "daily_cap": int(config.X_SESSION_DAILY_CAP),
        "daily_free_at": day_free,
        "window_exhausted": window_n >= int(config.X_SESSION_WINDOW_CAP),
        "daily_exhausted": day_n >= int(config.X_SESSION_DAILY_CAP),
    }


def record_session_metrics(con, posts, *, source="x_session", taken_at=None):
    """Записать метрики сессионного транспорта в ``metric_snapshot``.

    Поля: views, likes, replies, reposts, quotes (source='x_session',
    platform='x', external_id=tweet_id). Пишем только для постов, у которых
    уже есть строка ``content``: снапшот привязан к ``content_id``. Новые посты
    попадают сюда в том же прогоне из ``collect.store_posts``. Возвращает число
    записанных снапшотов.
    """
    taken = taken_at or now_iso()
    written = 0
    touched: list[int] = []
    for p in posts or []:
        ...
        if not tid:
            continue
        row = con.execute(
            "SELECT id FROM content WHERE platform='x' AND external_id=?",
            (tid,)).fetchone()
        if row is None:
            continue
        con.execute(
            """INSERT INTO metric_snapshot
                 (content_id, platform, external_id, captured_at, source,
                  views, likes, replies, reposts, quotes)
               VALUES (?,?,?,?,?,?,?,?,?,?)
               ON CONFLICT(content_id, captured_at) DO UPDATE SET
                 views=excluded.views, likes=excluded.likes, replies=excluded.replies,
                 reposts=excluded.reposts, quotes=excluded.quotes, source=excluded.source""",
            (row["id"], "x", tid, taken, source, p.get("views"), p.get("likes"),
             p.get("replies"), p.get("reposts"), p.get("quotes")))
        touched.append(int(row["id"]))
        written += 1
    con.commit()
    # ТЗ-43 (п. 2): общий путь расчёта дельт/корзины — заполняем сразу при
    # записи снимка (раньше это делал только YouTube). Сбой расчёта не имеет
    # права ронять сбор: метрики уже записаны.
    if touched:
        try:
            from tuber.core import metrics as core_metrics
            for cid in touched:
                core_metrics.apply_deltas_latest(con, cid)
            con.commit()
        except Exception:  # noqa: BLE001 — расчёт дельт не роняет сбор
            pass
        # D-66: просмотры X приходили в `metric_snapshot`, но НЕ доходили до
        # `content_latest` (устаревшие строки писали только лайки/ответы), из-за
        # чего оси «охват на подписчика» и «реакции на 1 000» строились без X.
        # Пересчитываем «последнее известное» из свежего снимка: `views` попадают
        # в `content_latest` тем же значением, что уже лежит в снимке. Сбой
        # пересчёта не имеет права ронять сбор (метрики уже записаны).
        try:
            for cid in touched:
                storage.recompute_content_latest(con, cid)
            con.commit()
        except Exception:  # noqa: BLE001 — оси не роняют сбор
            pass
    return written


#: Сколько последних снимков подписчиков хранить в ``source.meta_json``.
# TODO(debt-D-56): ряд роста подписчиков живёт в meta_json (обрезка 500
# точек), отдельной таблицы-ряда нет — см. TECH-DEBT.md D-56.
# С ТЗ-45 сама запись — общая функция ядра (одна форма ряда на все платформы);
# здесь оставлен ре-экспорт имени, чтобы прежний API X не менялся.
FOLLOWERS_HISTORY_MAX = storage.FOLLOWERS_HISTORY_MAX


def set_follower_snapshot(con, source_id, subs, *, avg_views=None, taken_at=None):
    """Снимок подписчиков аккаунта X (ТЗ-44) — делегирует в ядро (ТЗ-45).

    Обновляет ``source.subs``/``subs_at`` (+ ``avg_views`` при наличии данных)
    и дописывает точку в ``source.meta_json.followers_history`` общим ядровым
    :func:`tuber.core.storage.set_follower_snapshot`.
    """
    history = storage.set_follower_snapshot(
...
_COMPAT_STATEMENTS: list[str] = [
    ...
    SELECT cc.text_hash                                              AS text_hash,
           ...
           cc.lang                                                   AS lang,
           cc.title_ru                                               AS title_ru,
           cc.summary_ru                                             AS summary_ru,
           cc.status                                                 AS status,
           cc.method                                                 AS method,
           cc.model                                                  AS model,
    ...
    CREATE TEMP TRIGGER classified_ins INSTEAD OF INSERT ON classified BEGIN
      INSERT INTO classify_cache(text_hash, is_ai, topic, subtopic, claim_type, novelty,
          lang, title_ru, summary_ru, status, method, model, attempts, error, prompt_tokens,
      ...
      VALUES(NEW.text_hash, NEW.is_ai, NEW.topic, NEW.subtopic, NEW.claim_type,
          NEW.novelty, NEW.lang, NEW.title_ru, NEW.summary_ru,
          COALESCE(NEW.status, 'classified'), NEW.method,
      ...
      INSERT INTO classification(content_id, platform, external_id, is_ai, topic,
          subtopic, claim_type, novelty, lang, title_ru, summary_ru,
          status, method, model, attempts, error,
          prompt_tokens, completion_tokens, cost_usd, classified_at)
      SELECT c.id, 'x', c.external_id, NEW.is_ai, NEW.topic, NEW.subtopic,
          NEW.claim_type, NEW.novelty, NEW.lang, NEW.title_ru, NEW.summary_ru,
          COALESCE(NEW.status, 'classified'),
      ...
      ON CONFLICT(content_id) DO UPDATE SET
        ...
        title_ru = excluded.title_ru, summary_ru = excluded.summary_ru,
    ...
    CREATE TEMP TRIGGER classified_upd INSTEAD OF UPDATE ON classified BEGIN
      UPDATE classify_cache SET
        ...
        title_ru = NEW.title_ru, summary_ru = NEW.summary_ru,
