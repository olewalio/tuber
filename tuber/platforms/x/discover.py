...
def link_handles(links):
    """X-хендлы из ссылок (Р2.3, вид `link`).

    ТЗ-8 §2.1: классификация ссылки — одна точка правды
    (:func:`tuber.core.graph.classify_target`), дублирующего списка хостов здесь
    больше нет. Возвращаются только X-аккаунты: остальные типы целей
    (Telegram/YouTube/веб-фид) обрабатывает общий потребитель рёбер
    (`tuber graph consume`), а не X-дискавери.
    """
    out = []
    for u in links or []:
        target = core_graph.classify_target(u)
        if target.skip_reason:
            continue
        if target.target_platform == "x" and target.target_type == "account":
            out.append(target.target_value)
    seen, res = set(), []
    for h in out:
        if h not in seen:
            seen.add(h)
...
def verify_candidates(con, broker, *, max_verify=config.DISCOVERY_MAX_VERIFY_DEFAULT,
                      run_id=None, router=None):
    """Р3.1/Р3.2: проверить верхних кандидатов очереди (по приоритету).

    `router` (ТЗ-C Р4) прокидывается в `registry.verify_account`, чтобы при
    деградации Nitter недобранная лента добиралась аварийным резервом x_ssr.
    """
    queue = pending_candidates(con, limit=max_verify)
    results = {"checked": 0, "active": 0, "provisional": 0, "rejected": 0,
               "retry": 0, "blocked": 0, "reasons": {}, "details": []}
    for cand in queue:
        h = cand["handle"]
        res = registry.verify_account(con, broker, h, run_id=run_id, router=router)
        results["checked"] += 1
        st = res.get("status")
        if st == "active":
            results["active"] += 1
            ...
            results["retry"] += 1
            results["reasons"]["retry"] = results["reasons"].get("retry", 0) + 1
        elif st == "rejected":
            results["rejected"] += 1
