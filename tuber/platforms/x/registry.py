...
from .broker import NitterError, NitterTransportError
...
REASONS = {
    ...
    # ТЗ-C Р5: неудачная верификация — повтор, а не приговор
    "retry": "инстанс отдал < 15 item: повтор проверки (попытка не последняя)",
}
...
def promote_provisional(con, *, simulate_days=None, run_id=None, now=None):
    ...
    Путь (а) core_mentions доступен всегда (в т.ч. в фазе bootstrap, ТЗ-C Р1);
    путь (б) by_tenure — TENURE_DAYS дней непрерывного сбора. Дневной потолок (Р4.4) общий.
    """
    now = now or db.utcnow()
    ...
    for row in rows:
        ...
        # путь (а) core_mentions доступен ВСЕГДА, в том числе в фазе bootstrap
        # (ТЗ-C Р1): иначе замкнутый круг — active не растёт, bootstrap не
        # заканчивается, путь (а) выключен. Предохранители прежние:
        # VERIFY_MIN_MENTIONS (>=2 автора ядра) и DAILY_NEW_ACTIVE_CAP.
        if len(mentioners) >= config.VERIFY_MIN_MENTIONS:
...
# ------------------------------------------------- ТЗ-C Р5: счётчик попыток verify
def _verify_attempts(con, handle):
    """Сколько подряд неудачных верификаций (items_lt_15) уже было (candidate.meta_json)."""
    row = con.execute(
        "SELECT json_extract(COALESCE(meta_json, '{}'), '$.verify_attempts') AS n"
        " FROM candidate WHERE platform='x' AND handle=?", (handle,)).fetchone()
...
def _set_verify_attempts(con, handle, attempts):
    """Записать счётчик попыток в candidate.meta_json (строка уже существует)."""
    con.execute(
        "UPDATE candidate SET meta_json=json_set(COALESCE(meta_json, '{}'),"
        " '$.verify_attempts', ?) WHERE platform='x' AND handle=?",
        (int(attempts), handle))


def verify_account(con, broker, handle, *, run_id=None, apply_status=True, router=None):
    ...
    ТЗ-C Р4: если Nitter отдал меньше `VERIFY_MIN_ITEMS` записей, при его
    деградации лента добирается аварийным резервом x_ssr через уже существующую
    механику сбора (`collect._reserve_allowed` + `router.fetch_ssr`).
    ТЗ-C Р5: неудачная по items_lt_15 проверка повторяется до
    `VERIFY_MAX_ATTEMPTS` раз (счётчик в candidate.meta_json), а не отбраковывает
    читаемый аккаунт из-за одного сбоя инстанса.
    """
    h, err = validate_handle(handle)
    if err:
    ...
    # ТЗ-C Р4: Nitter отдал меньше VERIFY_MIN_ITEMS — при деградации добираем
    # ленту аварийным резервом x_ssr. Механика резерва берётся из collect
    # (не дублируется): _reserve_allowed + _fetch_ssr.
    reserve_used = False
    if items < config.VERIFY_MIN_ITEMS and router is not None:
        from . import collect as _collect
        reserve_err = NitterTransportError(f"items_lt_{config.VERIFY_MIN_ITEMS}")
        if _collect._reserve_allowed(router, reserve_err, reserve_used=0,
                                     max_per_run=config.XSSR_MAX_PER_RUN):
            extra = _collect._fetch_ssr(router, h) or []
            if extra:
                fetched, items, reserve_used = extra, len(extra), True
    ...
    promo_path = None
    retry_attempts = None
    not_found = (fetch_error is None and items == 0)
    if (apply_status and not prov_ok and prov_reason == "items_lt_15"
            and not not_found):
        # ТЗ-C Р5: сбой инстанса — повтор, а не приговор. Пока попыток меньше
        # VERIFY_MAX_ATTEMPTS, кандидат остаётся в очереди (retry), на N-й —
        # прежнее поведение (rejected).
        attempts = _verify_attempts(con, h) + 1
        retry_attempts = attempts
        if attempts < config.VERIFY_MAX_ATTEMPTS:
            new_status, reason = "candidate", "retry"
        else:
            new_status, reason = "rejected", "items_lt_15"
    else:
        if prov_ok:
            retry_attempts = 0  # успешная верификация сбрасывает счётчик
        if not prov_ok:
    ...
    with con:
        ...
        if daily_cap_hit or provisional_cap_hit:
            ...
            # ТЗ-C Р5: кандидат остаётся в очереди на повторную верификацию
            validated, reject_reason = "pending", "retry"
        ...
        # ТЗ-C Р5: счётчик попыток — в candidate.meta_json (после UPSERT, чтобы
        # строка кандидата уже существовала; триггер сохраняет прочие ключи).
        if retry_attempts is not None:
            _set_verify_attempts(con, h, retry_attempts)
    if new_status == "active":
        ...
        db.log_run(con, "INFO",
                   f"верификация не удалась (items_lt_15), попытка"
                   f" {retry_attempts}/{config.VERIFY_MAX_ATTEMPTS} — повтор",
                   handle=h, run_id=run_id)
    elif reason:
    ...
    return {"ok": new_status == "active" or new_status == "provisional",
            ...
            "reserve_used": reserve_used, "verify_attempts": retry_attempts}
