...
# ТЗ-C Р2: боевые пороги задаются средой, а не константами. Дефолты в
# config.py прежние (TENURE_DAYS=14, DISCOVERY_MAX_VERIFY_DEFAULT=30), откат —
# убрать эти строки (или экспортировать прежние значения).
export TUBER_X_TENURE_DAYS="${TUBER_X_TENURE_DAYS:-4}"
export TUBER_X_MAX_VERIFY="${TUBER_X_MAX_VERIFY:-60}"
...
# дискавери (только чтение БД: `store.connect`, без миграций и записи).
mapfile -t cfg < <("$TUBER_PYTHON" - <<'PY' 2>>"$LOG"
import os
from tuber.platforms.x import config, store, discover
remain = None
