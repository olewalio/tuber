...
#     суточные подписчики X на сессии).
...
LAUNCHERS=(
  ...
  "tuber_x_followers.sh:scripts/x/tuber_x_followers.sh::python3 -m tuber x followers"
  ...
  "tuber_tg_classify.sh:scripts/telegram/tuber_tg_classify.sh::python3 -m tuber tg classify"
  "tuber_tg_followers.sh:scripts/telegram/tuber_tg_followers.sh::python3 -m tuber tg followers"
  # --- общие (монорепо) ---
  ...
  "tuber_graph_first_movers.sh:scripts/common/tuber_graph_first_movers.sh::python3 -m tuber graph first-movers"
  "tuber_rating_slivki.sh:scripts/common/tuber_rating_slivki.sh::python3 -m tuber rating slivki capture"
  "tuber_queue_review.sh:scripts/common/tuber_queue_review.sh::python3 -m tuber queue review"
  "tuber_comments_daily.sh:scripts/common/tuber_comments_daily.sh::python3 -m tuber comments youtube + comments telegram + comments x"
  "tuber_trends_daily.sh:scripts/common/tuber_trends_daily.sh::python3 -m tuber trends inside + trends novelties --save"
  "tuber_digest_slivki.sh:scripts/common/tuber_digest_slivki.sh::python3 -m tuber digest slivki --send"
  "tuber_metric_queue.sh:scripts/common/tuber_metric_queue.sh::python3 -m tuber metrics run --allow-production"
)
...
JOBS=(
  ...
  "tuber_tg_classify.sh:50 */2 * * *:Telegram: классификация постов моделью"
)
...
NEW_JOBS=(
  ...
  "tuber_tg_followers.sh:20 3 * * *:Tuber-Telegram: подписчики (публичная веб-страница)"
  "tuber_graph_first_movers.sh:20 7 * * *:Tuber: граф первопроходцев (trusted_indegree, lead_time, first_mover)"
  "tuber_rating_slivki.sh:35 7 * * *:Tuber: рейтинг «сливки» (суточный снимок метрик автора)"
  "tuber_queue_review.sh:50 7 * * *:Tuber: разбор очереди кандидатов (вердикты promote/hold/reject)"
  "tuber_comments_daily.sh:30 8 * * *:Tuber: комментарии — YouTube (топ виральных) + Telegram (Telethon) + сигнал X"
  "tuber_trends_daily.sh:55 7 * * *:Tuber: подтемы сюжетов и новинки (HN/GitHub/Product Hunt/arXiv)"
  "tuber_digest_slivki.sh:0 9 * * *:Tuber: утренняя выдача «Сливки» (10 авторов + 5 подтем + 5 новинок + 3 обсуждения) в Telegram"
  "tuber_metric_queue.sh:25 * * * *:Tuber: очередь замеров 1/6/24/72 ч (X, Telegram) — скорость и блок «Раннее»"
)
...
install_one() {
  ...
  rm -f "$dest"   # снять прежний симлинк/файл строго по имени
  {
    ...
    # Обёртки, умеющие показывать план (TUBER_LAUNCHER_DRYRUN), рисуют его сами
    # и с учётом окружения (например, доставка «Сливок»). Прочие — печатаем цель.
    if grep -qF 'TUBER_LAUNCHER_DRYRUN' "$full"; then
      printf '  exec %s\n' "$full"
    else
      printf '  printf "%%s\\n" "%s"\n' "$cmd"
      printf '  exit 0\n'
