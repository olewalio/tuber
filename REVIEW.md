...
Обновление среза: в репозитории **441 файл**, волны ТЗ-1…ТЗ-54, история разработки —
138 коммитов (16–22.09.2026). Публикация — один корневой коммит: история боевого
каталога внутрь не переносится, поэтому в этом репозитории ровно один коммит.

- Платформы: `tuber/platforms/{youtube,x,telegram}` на общем ядре `tuber/core`;
  112 python-файлов вне тестов, 134 тестовых файла.
- Тесты на этом срезе: `python3 -m pytest -q` → **1 616 passed, 12 skipped** (110 с).
  12 пропущенных требуют боевых баз и размеченного набора `tests/data/` — в публикацию
  их не кладут; в боевом каталоге тот же набор даёт 1 627 passed, 1 xfailed.
  Единственный xfail — осознанный долг, разобран в `docs/TZ-40-known-debt-test-tz6.md`.
- Личные данные: адрес доставки «Сливок» и субъект «где я» читаются из окружения
  (`/root/.hermes/tuber_owner.env`, вне репозитория); личных значений в коде нет,
  за этим следит `tests/test_no_personal_data.py`.
- Постановки: `docs/TZ-*.md`; из последних добавленных — ТЗ-37…ТЗ-52 (инварианты
  компакта выдачи, отказ от Telegram в компакте, нумерация разделов, известный долг теста).
- Долги: `TECH-DEBT.md`, нумерация D-01…D-70, у каждого пункта явный статус
  «открыт/закрыт».
...
12. `TECH-DEBT.md` — осознанные компромиссы и пробелы (D-01…D-70).
