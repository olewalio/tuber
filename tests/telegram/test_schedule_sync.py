"""ТЗ-20 Р2 (долг D-01): расписание живёт в двух местах — не дать им разойтись.

Перенос `tests/test_schedule_sync.py` проекта tuber-telegram (ТЗ-4): установщик
переехал в `scripts/telegram/install_hermes_cron.sh`, шимы — в тот же каталог.

Установщик `scripts/install_hermes_cron.sh` объявляет расписание заданий, а
планировщик Hermes хранит фактическое. В ТЗ-11 (проект tuber-x) расхождение по 4
заданиям из 10 нашлось только ручной сверкой. Тест закрывает этот разрыв.

Как устроен:
  * единый источник — блок объявления заданий `JOBS=(...)` в установщике;
  * фактические задания проекта читаются из конфигурации планировщика Hermes
    (`/root/.hermes/cron/jobs.json`);
  * расписание и наличие каждого задания сверяются попарно; расхождение роняет
    тест с сообщением, где именно и что ожидалось;
  * отсутствие конфигурации планировщика (другая машина, чистая копия) — не
    падение, а явный `skip` с причиной;
  * сравнение проверяется и на синтетике, чтобы гарантированно ловить именно
    случай «то же задание, другое время».

Пути переопределяются переменными окружения `TUBER_TG_INSTALLER` и
`TUBER_TG_HERMES_JOBS_JSON` — приёмка подставляет временную копию установщика с
...
INSTALLER = os.environ.get(
    "TUBER_TG_INSTALLER",
    os.path.join(ROOT, "scripts", "telegram", "install_hermes_cron.sh"),
)
...
# Задания Tuber-Telegram в планировщике: все шимы проекта носят префикс
# tuber_telegram_ (сбор постов + три задания моста).
PROJECT_PREFIXES = ("tuber_telegram_",)
PROJECT_EXTRA = set()


def is_project_script(base):
    return base.startswith(PROJECT_PREFIXES) or base in PROJECT_EXTRA
...
def parse_installer_jobs(path):
    ...
    Строка вида `"tuber_telegram_collect.sh:*/30 * * * *:Tuber-Telegram: сбор"`.
...
def test_schedule_matches_scheduler_config():
    ...
    assert actual, f"в {JOBS_JSON} нет ни одного задания tuber_telegram_*.sh"
    problems = compare_schedules(declared, actual)
    assert problems == [], (
        "расписание разошлось с установщиком; единый источник — блок"
        " JOBS=(...) в scripts/install_hermes_cron.sh:\n  - "
        + "\n  - ".join(problems))


def test_comparison_catches_same_job_other_time():
    """Р2.3: случай «то же задание, другое время» — тест обязан поймать."""
    problems = compare_schedules({"tuber_telegram_feed_export.sh": "30 6 * * *"},
                                 {"tuber_telegram_feed_export.sh": "30 7 * * *"})
    assert len(problems) == 1
    assert "tuber_telegram_feed_export.sh" in problems[0]
    assert "'30 6 * * *'" in problems[0] and "'30 7 * * *'" in problems[0]


def test_comparison_catches_missing_and_extra():
    """Наличие задания тоже сверяется: пропажа и лишнее — обе ошибки."""
    missing = compare_schedules({"tuber_telegram_discover.sh": "40 7 * * *"}, {})
    assert missing and "не зарегистрировано" in missing[0]
    extra = compare_schedules({}, {"tuber_telegram_feed_export.sh": "30 6 * * *"})
