...
# ---------------------------------------------------------------------------
# 11. Битая ссылка (ТЗ №2 ч.2): не роняет парсинг поста/канала
# ---------------------------------------------------------------------------
def test_broken_link_does_not_raise():
    samples = ("https://[::1", "http://[bad",
               "ok https://example.com/a", "[текст](https://x.com/a)")
    # Разборщик ссылок не бросает ни на одной строке.
    for u in samples:
        C._safe_urlsplit(u)
    assert C._safe_urlsplit("https://[::1") is None
    assert C._safe_urlsplit("http://[bad") is None
    # Битые ссылки отбрасываются, счётчик фиксирует их, пост не падает.
    ext, bad = C._split_external_links(list(samples))
    assert bad == 2
    assert "https://[::1" not in ext and "http://[bad" not in ext
    assert any("example.com" in u for u in ext)
