...
def test_run_fetches_max_and_keeps_top_likes(conn):
    ...
    assert summary["comments"] == config.COMMENT_MAX_PER_VIDEO
    rows = conn.execute(
    ...
    expected = [c["comment_id"] for c in
                sorted(items, key=lambda c: -c["likes"])[:config.COMMENT_MAX_PER_VIDEO]]
    assert got == expected
    # Сохранены не первые по порядку следования, а именно лучшие по лайкам.
    assert got != [c["comment_id"] for c in items[:config.COMMENT_MAX_PER_VIDEO]]
