...
def test_verify_account_retries_then_rejects_unreadable(con, make_broker):
    """ТЗ-C Р5: items_lt_15 не отбраковывает с одного сбоя — 3 попытки, потом reject.

    Попытки 1 и 2 -> кандидат остаётся в очереди (status new/candidate, validated
    pending, reason retry), source НЕ rejected; 3-я -> прежнее поведение.
    """
    _seed_core(con)
    b = make_broker()
    ...
    for attempt in (1, 2):
        ...
        cand = con.execute("SELECT * FROM candidates WHERE handle='cand'").fetchone()
        assert cand["validated"] == "pending" and cand["reject_reason"] == "retry"

    res = registry.verify_account(con, b, "cand")
    assert res["status"] == "rejected" and res["reason"] == "items_lt_15"
    assert res["verify_attempts"] == config.VERIFY_MAX_ATTEMPTS
    assert con.execute("SELECT status FROM accounts WHERE handle='cand'"
                       ).fetchone()[0] == "rejected"
    cand = con.execute("SELECT * FROM candidates WHERE handle='cand'").fetchone()
