...
def _hermes(tmp_path, scripts=None, lines=(), name="agent.log"):
    """Конфигурация планировщика + каталог журналов с заданным содержимым."""
    if scripts is None:
        scripts = health.REPORT_JOB_SCRIPT
...
def test_report_job_ids_supports_both_scripts(tmp_path):
    """ТЗ-C: проверка следит и за владельческой сводкой, и за legacy-скриптом."""
    cfg = tmp_path / "jobs.json"
    cfg.write_text(json.dumps({"jobs": [
        {"id": "owner01", "script": "/root/.hermes/scripts/tuber_report.sh"},
        {"id": "local01", "script": "tuber_x_report.sh"},
        {"id": "other", "script": "tuber_x_collect_a.sh"},
    ]}), encoding="utf-8")
    ids = health._report_job_ids(str(cfg), health.REPORT_JOB_SCRIPT)
    assert ids == ["owner01", "local01"]
    # строковая форма сохранена (обратная совместимость)
    assert health._report_job_ids(str(cfg), "tuber_report.sh") == ["owner01"]
