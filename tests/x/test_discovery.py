...
def test_extract_candidates():
    ...
    # ТЗ-8 Р1.2: ссылка на /status/<id> — ребро на пост, не на аккаунт, поэтому
    # кандидатом-аккаунтом она больше не становится (единая точка правды
    # classify_target).
    assert ("delta", "link") not in cands
    ...
    # ссылка twitter.com/<handle>/status/<id> — тоже ребро на пост (ТЗ-8 Р1.2)
    t = by_owner["theta"]
    tc = {(h, k) for h, k, _ in discover.extract_candidates(t, "инвестиции и раунды")}
    assert ("eta", "link") not in tc
...
def test_promote_path_a_during_bootstrap(con):
    """ТЗ-C Р1: путь (а) core_mentions работает и в фазе bootstrap.

    Раньше provisional с двумя упоминаниями ядра НЕ промоутился при active < 20
    (замкнутый круг). Теперь круг разорван: путь (а) доступен всегда, а
    предохранители (VERIFY_MIN_MENTIONS, DAILY_NEW_ACTIVE_CAP) сохранены.
    """
    for core in ("core1", "core2"):
        con.execute("INSERT INTO accounts (handle, tier, status, added_by)"
                    " VALUES (?, 'A', 'active', 'test')", (core,))
    ...
    assert any(p["handle"] == "p1" and p["by"] == "core_mentions" for p in promoted), promoted
    ...
    # фаза bootstrap завершилась бы только при >=20 active; п1 один active
    assert registry.bootstrap_phase(con) is True
