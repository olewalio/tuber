...
# --------------------------- ТЗ-43A: контракт поста сессионного транспорта X
def test_session_post_matches_post_contract():
    """Пост сессии X обязан нести контракт поста (как ``broker.parse_rss``).

    Контракт тот же, плюс метрики сессии (likes/replies/reposts/views/quotes/
    author_followers/author_id). Лишние ключи потребителей не ломают.
    """
    from tuber.platforms.x import session as xs

    info = {
        "id": "111", "author": "nasa", "author_id": "42",
        "author_followers": 7, "published_at_utc": "2026-09-20T10:00:00",
        "published_src": "x_session", "text": "текст", "likes": 3, "replies": 1,
        "retweets": 2, "quotes": 0, "views": 999, "is_retweet": False,
        "is_quote": False, "is_reply": False, "links": [], "mentions": [],
        "hashtags": [], "media_kind": None, "orig_handle": None,
    }
    post = xs.XSessionTransport.to_post(info, owner_handle="nasa")
    ...
    for key in required:
        assert key in post, f"контракт поста: нет ключа {key}"
    assert post["tweet_id"] == "111" and post["owner_handle"] == "nasa"
    assert post["published_src"] == "x_session"
    assert (post["views"], post["likes"], post["reposts"], post["quotes"]) == \
        (999, 3, 2, 0)
    assert post["author_followers"] == 7 and post["author_id"] == "42"

    # Ключи контракта сессии — надмножество ключей RSS-парсера.
    rss_keys = set()
    try:
        from tuber.platforms.x.broker import parse_rss
        sample = ("<rss><channel><item><title>t</title>"
                  "<dc:creator>@nasa</dc:creator>"
                  "<guid>111</guid><pubDate>Sun, 20 Sep 2026 10:00:00 GMT</pubDate>"
                  "</item></channel></rss>")
        ...
        rss_keys = set()
    if rss_keys:
        assert rss_keys <= set(post), f"контракт сессии уже RSS: {rss_keys - set(post)}"
