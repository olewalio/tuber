...
def test_post_metrics_history_table_and_index(db_path):
    ...
    # Повторный замер того же поста в тот же слот должен ОБНОВЛЯТЬ снапшот,
    # а не падать с IntegrityError (ТЗ №2 ч.1).
    ...
    con.commit()
    # Повторная запись той же пары (tweet_id, taken_at) не бросает и обновляет метрики.
    con.execute("INSERT INTO post_metrics_history (tweet_id, taken_at, likes)"
                " VALUES ('1','2026-09-15T10:00:00',6)")
    con.commit()
    rows = con.execute(
        "SELECT likes FROM post_metrics_history WHERE tweet_id='1'"
        " AND taken_at='2026-09-15T10:00:00'"
    ).fetchall()
    assert len(rows) == 1
    assert rows[0][0] == 6
    con.close()
