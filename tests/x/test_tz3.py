...
# ------------------------------------------------------------------- ТЗ-B
def test_classify_cap_from_env(con, monkeypatch):
    """ТЗ-B: потолок читается из `TUBER_X_CLASSIFY_DAILY_CAP`.

    Default без переменной — 400 (безопасный откат); с переменной = её значение.
    Проверяем и разбор env (изолированным процессом), и то, что тот же потолок
    реально ограничивает выборку прогона.
    """
    import os
    import subprocess
    import sys
    ...
    code = "from tuber.platforms.x import config; print(config.CLASSIFY_DAILY_CAP)"
    env = dict(os.environ)
    env.pop("TUBER_X_CLASSIFY_DAILY_CAP", None)
    default = subprocess.check_output([sys.executable, "-c", code],
                                      cwd=root, env=env).strip()
    env["TUBER_X_CLASSIFY_DAILY_CAP"] = "7"
    seven = subprocess.check_output([sys.executable, "-c", code],
                                    cwd=root, env=env).strip()
    assert default == b"400", "без env потолок обязан быть 400 (откат)"
    assert seven == b"7", "env обязан переопределять потолок"

    a = acc(con, "a")
    for i in range(10):
        post(con, a, f"{i}", f"OpenAI model number {i} release for agents", handle="a")
    monkeypatch.setattr(config, "CLASSIFY_DAILY_CAP", 7)
    s = classify.run(con, client=FakeModel(), check_budget=False)
    assert s["selected"] == 7 and s["classified"] == 7


def test_classify_usd_cap_stops_not_crashes(con, monkeypatch):
    """ТЗ-B (G2): исчерпанный $ лимит стадии — мягкий стоп, не исключение."""
    day = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    a = acc(con, "a")
    post(con, a, "1", "OpenAI model release with agents", handle="a")
    con.execute("INSERT INTO classify_daily (day, posts, cost_usd)"
                " VALUES (?, 0, ?)", (day, 5.0))
    con.commit()
    monkeypatch.setattr(config, "CLASSIFY_DAILY_USD_CAP", 1.0)
    fake = FakeModel()
    s = classify.run(con, client=fake, check_budget=False)  # не должно бросить
    assert s["budget_ok"] is False
    assert s["classified"] == 0 and fake.calls == 0
    assert "лимит стадии" in s["budget_reason"]


def test_classify_failure_keeps_pending(con):
    """ТЗ-B: отказ канала не роняет прогон, посты остаются в pending."""
    from tuber.platforms.x.channels import DeepSeekError

    a = acc(con, "a")
    tids = [post(con, a, str(i), f"OpenAI model {i} release for agents", handle="a")
            for i in range(3)]

    class Boom(FakeModel):
        def classify(self, *args, **kwargs):
            raise DeepSeekError("429: слишком много запросов")

    s = classify.run(con, client=Boom(), check_budget=False)  # без исключения
    assert s["failed"] == 3 and s["classified"] == 0
    still = {p["tweet_id"] for p in classify.select_pending(con)}
    assert set(tids) <= still, "неудачные посты обязаны остаться в хвосте"
