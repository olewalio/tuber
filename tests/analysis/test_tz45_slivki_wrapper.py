"""ТЗ-45: обёртка суточного рейтинга «сливки».

Проверяем главное поведение ``scripts/common/tuber_rating_slivki.sh``:
...
WRAPPER = os.path.join(ROOT, "scripts", "common", "tuber_rating_slivki.sh")

FAKE = """#!/usr/bin/env bash
printf '%s\\n' "$@" >> "$TUBER_RATING_SLIVKI_RECORD"
if [ "${TUBER_RATING_SLIVKI_FAKE_RC:-0}" != "0" ]; then
  echo "boom" >&2
  exit "$TUBER_RATING_SLIVKI_FAKE_RC"
fi
if [ "${TUBER_RATING_SLIVKI_FAKE_EMPTY:-0}" = "1" ]; then exit 0; fi
printf '{"snapshot": {"rows": 9492}}\\n'
"""
...
def _run(tmp_path, *, rc="0", empty="0", dryrun=False, db=""):
    ...
    env.update({
        ...
        "TUBER_RATING_SLIVKI_PROJECT": str(project),
        "TUBER_RATING_SLIVKI_LOG_DIR": str(tmp_path / "logs"),
        "TUBER_RATING_SLIVKI_RECORD": str(record),
        "TUBER_RATING_SLIVKI_FAKE_RC": rc,
        "TUBER_RATING_SLIVKI_FAKE_EMPTY": empty,
        "TUBER_RATING_SLIVKI_TIMEOUT_SEC": "5",
    })
    if db:
        env["TUBER_DB"] = db
    if dryrun:
...
def test_dryrun_prints_plan_without_cli(tmp_path):
    ...
    assert proc.stdout.strip() == (
        f"{tmp_path}/bin/python3 -m tuber rating slivki capture --allow-production")
